...
class TestPromptBuilderConstants:
    ...
    def test_api_server_hint_scopes_media_tag_guidance(self):
        """The API server owns MEDIA tag conversion for remote clients."""
        hint = PLATFORM_HINTS["api_server"]
        assert "MEDIA:" in hint
        assert "inlined" in hint.lower()
        assert "download links" in hint.lower()
        assert "login-protected" in hint.lower()
        assert "never expose" in hint.lower()
        assert "ssh" in hint.lower()
